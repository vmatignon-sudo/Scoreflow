  ...
  return (
    <aside className="hidden sm:flex fixed left-0 top-0 h-screen w-[240px] bg-white/90 glass shadow-lg flex-col z-30">
      ...
      <div className="px-3 pb-3">
        <Link href="/deals/new"
          className="inline-flex items-center justify-center gap-1.5 w-full bg-[#1d1d1f] text-white py-[7px] rounded-[10px] text-[12px] font-medium hover:bg-black active:scale-[0.98] transition-all">
          ...
          {navigation.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <item.icon className="w-[17px] h-[17px]" strokeWidth={isActive ? 1.8 : 1.5} />
                  {item.name}
                </Link>
              </li>
            );

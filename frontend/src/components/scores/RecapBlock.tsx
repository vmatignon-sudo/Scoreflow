          ...
          {view === 'rosace' ? (
            <svg viewBox="0 0 360 360" width="320" height="320" style={{ display: 'block' }}>
              ...
              {AXES.map((label, i) => {
                ...
                if (i === 0) {
                  labelEl = (<text x={CX} y={CY - R - 8} textAnchor="middle" dominantBaseline="central" style={{ fontSize: '11px', fill: '#6e6e73', fontWeight: 500 }}>{label}</text>);
                } else if (i === 2) {
                  labelEl = (<text x={CX} y={CY + R + 12} textAnchor="middle" dominantBaseline="central" style={{ fontSize: '11px', fill: '#6e6e73', fontWeight: 500 }}>{label}</text>);
                } else if (i === 1) {
                  labelEl = (<text x={CX + R + 10} y={CY} textAnchor="middle" dominantBaseline="central" transform={`rotate(90, ${CX + R + 10}, ${CY})`} style={{ fontSize: '11px', fill: '#6e6e73', fontWeight: 500 }}>{label}</text>);
                } else {
                  labelEl = (<text x={CX - R - 10} y={CY} textAnchor="middle" dominantBaseline="central" transform={`rotate(-90, ${CX - R - 10}, ${CY})`} style={{ fontSize: '11px', fill: '#6e6e73', fontWeight: 500 }}>{label}</text>);
                }
                return (
